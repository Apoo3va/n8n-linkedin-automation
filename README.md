...
LinkedIn's API requires images to be uploaded via a separate "register upload" call before referencing them — the built-in n8n LinkedIn node handles this automatically for image posts. Rate limits apply, so don't schedule this to post too frequently.
